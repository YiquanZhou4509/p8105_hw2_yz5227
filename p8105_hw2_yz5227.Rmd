...
prof.data <- read_excel("./202409 Trash Wheel Collection Data.xlsx", sheet = "Professor Trash Wheel", range = "A2:M120")
```

```{r}
#Clean prof name
prof.data <- janitor::clean_names(prof.data)
...
```{r}
#load the Gwynnda Trash Wheel sheet with the correct range
gwy.data <- read_excel("./202409 Trash Wheel Collection Data.xlsx", sheet = "Gwynnda Trash Wheel", range = "A2:L265")
```

```{r}
#Clean gwy name
gwy.data <- janitor::clean_names(gwy.data)
...
```{r}
#merge datasets
tidy_trash <- bind_rows(trash.data, prof.data, gwy.data)

#Make trash_wheel as heading 
tidy_trash <- tidy_trash %>%
...
```{r}
#Load Data
price.data <- read_csv("./zillow_data/Zip_zori_uc_sfrcondomfr_sm_month_NYC.csv")
```
